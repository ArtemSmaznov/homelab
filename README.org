...
#+begin_src yaml
  db_digikam:
    ...
    ports:
      - ${DIGIKAM_PORT}:3306
    volumes:
      - ${VOLUMES_DIR}/digikam/db:/var/lib/mysql
    environment:
      # application
...
#+begin_src yaml
  db_immich:
    ...
    volumes:
      - ${VOLUMES_DIR}/immich/db:/var/lib/postgresql/data
    environment:
      # application
...
#+begin_src yaml
  influxdb:
    ...
    volumes:
      - ${VOLUMES_DIR}/grafana/influxdb:/var/lib/influxdb2
    networks:
      - monitor
      - hass
#+end_src
...
#+begin_src yaml
  db_nextcloud:
    ...
    volumes:
      - ${VOLUMES_DIR}/nextcloud/db:/var/lib/mysql
    environment:
      # application
...
#+begin_src yaml
  db_webtrees:
    ...
    command: --transaction-isolation=READ-COMMITTED --log-bin=binlog --binlog-format=ROW
    volumes:
      - ${VOLUMES_DIR}/webtrees/db:/var/lib/mysql
    environment:
      # application
      MARIADB_AUTO_UPGRADE: 1
      MARIADB_DISABLE_UPGRADE_BACKUP: 1
