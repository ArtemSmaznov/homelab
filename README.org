...
- [[#observability][Observability]]
  ...
  - [[#apc-ups][APC UPS]]
  - [[#pricebuddy-1][PriceBuddy]]
- [[#automation][Automation]]
  - [[#home-assistant-1][Home Assistant]]
- [[#documents][Documents]]
  - [[#nextcloud-2][Nextcloud]]
  - [[#books][Books]]
  - [[#webtrees-1][WebTrees]]
- [[#music][Music]]
  - [[#hold-mpd][HOLD MPD]]
...
secrets:
  ...
  pricebuddy_db_password:
    file: secrets/pricebuddy/db_password
  pricebuddy_db_root_password:
    file: secrets/pricebuddy/db_root_password
#+end_src
...
#+begin_src yaml
  db_hass:
    container_name: db_hass
    depends_on:
      - hass
    <<: *service_default
    image: influxdb:${INFLUXDB_VERSION}
    ports:
      - ${DB_INFLUXDB_LOCAL_PORT}:8086
    volumes:
      - ${INFLUXDB_DIR}/hass:/var/lib/influxdb2
    networks:
      - monitor
      - hass
#+end_src
...
#+begin_src yaml
  db_immich:
    container_name: db_immich
    <<: *service_default
    image: ghcr.io/immich-app/postgres:14-vectorchord0.3.0-pgvectors0.2.0
    volumes:
      - ${POSTGRESQL_DIR}/immich:/var/lib/postgresql/data
    environment:
      # application
...
#+begin_src yaml
  db_nextcloud:
    ...
    volumes:
      - ${MYSQL_DIR}/nextcloud:/var/lib/mysql
    environment:
      # application
...
#+begin_src yaml
  db_pricebuddy:
    container_name: db_pricebuddy
    <<: *mariadb
    ports:
      - ${DB_PRICEBUDDY_LOCAL_PORT}:3306
    volumes:
      - ${MYSQL_DIR}/pricebuddy:/var/lib/mysql
    environment:
      # application
      MARIADB_DATABASE: ${PRICEBUDDY_DB_NAME}
      MARIADB_USER: ${PRICEBUDDY_DB_USER}
      MARIADB_PASSWORD_FILE: /run/secrets/pricebuddy_db_password
      MARIADB_ROOT_PASSWORD_FILE: /run/secrets/pricebuddy_db_root_password
    secrets:
      - pricebuddy_db_password
      - pricebuddy_db_root_password
    networks:
...
#+begin_src yaml
  db_webtrees:
    container_name: db_webtrees
    <<: *mariadb
    command: --transaction-isolation=READ-COMMITTED --log-bin=binlog --binlog-format=ROW
    volumes:
      - ${MYSQL_DIR}/webtrees:/var/lib/mysql
    environment:
      # application
      MARIADB_AUTO_UPGRADE: 1
      MARIADB_DISABLE_UPGRADE_BACKUP: 1
...
#+begin_src yaml
  proxy:
    ...
    ports:
      ...
      - 443:443
    volumes:
      - ${VOLUMES_DIR}/proxy/certs:/etc/nginx/certs:ro
      - ${VOLUMES_DIR}/proxy/vhost.d:/etc/nginx/vhost.d
      - ${VOLUMES_DIR}/proxy/html:/usr/share/nginx/html
...
#+begin_src yaml
  acme:
    ...
    depends_on:
      - proxy
    volumes:
      - ${VOLUMES_DIR}/proxy/certs:/etc/nginx/certs
      - ${VOLUMES_DIR}/proxy/acme:/etc/acme.sh
      - ${VOLUMES_DIR}/proxy/vhost.d:/etc/nginx/vhost.d
      - ${VOLUMES_DIR}/proxy/html:/usr/share/nginx/html
...
#+begin_src yaml :tangle no
  wireguard-ui:
    ...
    cap_add:
      - NET_ADMIN
    volumes:
      - ${VOLUMES_DIR}/wireguard-ui:/app/db
      - ${VOLUMES_DIR}/wireguard:/etc/wireguard
    environment:
      # app
      SENDGRID_API_KEY:
      EMAIL_FROM_ADDRESS:
...
#+begin_src yaml
  transmission:
    ...
    ports:
      ...
      - ${TRANSMISSION_PEER_PORT}:${TRANSMISSION_PEER_PORT}/udp
    volumes:
      - ${VOLUMES_DIR}/transmission/config:/config
      - ${VOLUMES_DIR}/transmission/watch:/watch
      - ${VOLUMES_DIR}/transmission/downloads:/downloads
...
#+begin_src yaml
  transmission:
    ...
    volumes:
      - ${VOLUMES_DIR}/transmission/config:/config/transmission-home
...
#+begin_src yaml
  home:
    ...
    container_name: home
    volumes:
      - ${VOLUMES_DIR}/heimdall:/config/www
    environment:
      # proxy
...
#+begin_src yaml
  grafana:
    ...
    volumes:
      - ${VOLUMES_DIR}/grafana/app:/var/lib/grafana
    environment:
      # proxy
...
#+begin_src yaml
  prometheus:
    ...
    volumes:
      - ${VOLUMES_DIR}/prometheus/config:/etc/prometheus
      - ${VOLUMES_DIR}/prometheus/data:/prometheus
      - /var/run/docker.sock:/var/run/docker.sock:ro
    networks:
      - monitor
#+end_src
...
#+begin_src yaml
  prometheus-zfs:
    <<: *service_default
    image: matusnovak/prometheus-zfs:${PROMETHEUS_ZFS_VERSION}
    container_name: zfs
    hostname: zfs
    privileged: true
    ports:
      - ${PROMETHEUS_ZFS_LOCAL_PORT}:9901
    networks:
      - monitor
#+end_src
...
#+begin_src yaml
  prometheus-node-exporter:
    ...
    pid: host
    ports:
      - ${PROMETHEUS_NODE_EXPORTER_LOCAL_PORT}:9100
    volumes:
      - /:/host:ro,rslave
    networks:
      - monitor
...
*** APC UPS exporter
#+begin_src yaml
  prometheus-apc-exporter:
    <<: *service_default
    image: jangrewe/apcupsd-exporter:${PROMETHEUS_APC_EXPORTER_VERSION}
    container_name: apc_exporter
    hostname: apc_exporter
    depends_on:
      - apc
    command: -apcupsd.addr apc:3551
    ports:
      - ${PROMETHEUS_APC_EXPORTER_LOCAL_PORT}:9162
    networks:
      - monitor
#+end_src
...
#+begin_src yaml
  apc:
    <<: *service_default
    image: gregewing/apcupsd:${APC_VERSION}
    container_name: apc
    hostname: apc
    ports:
      - ${APC_LOCAL_PORT}:3551
    volumes:
      ...
      - /run/apcupsd:/run/apcupsd
      - ${VOLUMES_DIR}/apc:/etc/apcupsd
    devices:
      - /dev/usb/hiddev0
    environment:
      TZ: ${TIME_ZONE}
...
#+begin_src yaml
  pricebuddy:
    ...
    <<: *service_default
    image: jez500/pricebuddy:${PRICEBUDDY_VERSION}
    depends_on:
      db_pricebuddy:
        condition: service_healthy
    ports:
      - ${PRICEBUDDY_LOCAL_PORT}:80
    volumes:
      - ${VOLUMES_DIR}/pricebuddy:/app/storage
    environment:
      # application
      APP_KEY: ${PRICEBUDDY_KEY}
      APP_URL: https://${PRICEBUDDY_HOST}
      ASSET_URL: https://${PRICEBUDDY_HOST}
      SCRAPER_BASE_URL: http://scraper_pricebuddy:3000

      # database
      DB_HOST: db_pricebuddy
      DB_DATABASE: ${PRICEBUDDY_DB_NAME}
      DB_USERNAME: ${PRICEBUDDY_DB_USER}
      # DB_PASSWORD_FILE: /run/secrets/pricebuddy_db_password
      DB_PASSWORD: ${PRICEBUDDY_DB_PASSWORD}

      # proxy
      VIRTUAL_HOST: ${PRICEBUDDY_HOST}
      VIRTUAL_PORT: 80
      LETSENCRYPT_HOST: ${PRICEBUDDY_HOST}
      LETSENCRYPT_EMAIL: ${MY_EMAIL}
    networks:
      - pricebuddy
      - proxy-tier
...
#+begin_src yaml
  scraper_pricebuddy:
    container_name: scraper_pricebuddy
    <<: *service_default
    image: jez500/seleniumbase-scrapper:${SCRAPER_PRICEBUDDY_VERSION}
    healthcheck:
      test: ["CMD", "curl", "--fail", "http://0.0.0.0:3000/health"]
      interval: 30s
      timeout: 10s
      retries: 3
      start_period: 5s
    ports:
      - ${SCRAPER_PRICEBUDDY_LOCAL_PORT}:3000
    networks:
      - pricebuddy
#+end_src
...
#+begin_src yaml
  hass:
    ...
    volumes:
      - ${VOLUMES_DIR}/home-assistant/config:/config
...
#+begin_src yaml
  zwave-js-ui:
    ...
    volumes:
      - ${VOLUMES_DIR}/home-assistant/zwave:/usr/src/app/store
    devices:
...
#+begin_src yaml
  whisper:
    ...
    ports:
      - 10300:10300
    volumes:
      - ${VOLUMES_DIR}/home-assistant/wyoming-whisper:/data
...
#+begin_src yaml
  piper:
    ...
    ports:
      - 10200:10200
    volumes:
      - ${VOLUMES_DIR}/home-assistant/wyoming-piper:/data
...
#+begin_src yaml
  nextcloud-app:
    ...
    depends_on:
      ...
      - redis_nextcloud
    volumes:
      - ${VOLUMES_DIR}/nextcloud/app:/var/www/html
      # - ${VOLUMES_DIR}/nextcloud/opcache-recommended.ini:/usr/local/etc/php/conf.d/opcache-recommended.ini
      - ${DOWNLOADS_DIR}:/var/www/html/data/artem/files/downloads/amadeus
      - ${MUSIC_DIR}:/var/www/html/data/artem/files/music
      - ${PICTURES_DIR}:/var/www/html/data/artem/files/pictures
      - ${PHOTOS_DIR}:/var/www/html/data/artem/files/photos
      - ${VIDEOS_DIR}:/var/www/html/data/artem/files/videos
      - type: tmpfs
        target: /tmp:exec
    environment:
      # app
...
#+begin_src yaml
  nextcloud-server:
    ...
    volumes:
      - ${VOLUMES_DIR}/nextcloud/app:/var/www/html:ro
    environment:
      # proxy
...
#+begin_src yaml
  cron_nextcloud:
    ...
    volumes:
      - ${VOLUMES_DIR}/nextcloud/app:/var/www/html
      - ${VOLUMES_DIR}/nextcloud/cron:/var/spool/cron/crontabs
    networks:
      - nextcloud
#+end_src
...
#+begin_src yaml
  kavita:
    ...
    <<: *service_default
    image: lscr.io/linuxserver/kavita:${KAVITA_VERSION}
    ports:
      - ${KAVITA_LOCAL_PORT}:5000
    volumes:
      - ${VOLUMES_DIR}/kavita/config:/config
      - ${LIBRARY_DIR}:/data
    environment:
      # application
      PUID: ${UID}
      ...
      # proxy
      VIRTUAL_HOST: ${BOOKS_HOST}
      VIRTUAL_PORT: 5000
      LETSENCRYPT_HOST: ${BOOKS_HOST}
      LETSENCRYPT_EMAIL: ${MY_EMAIL}
    networks:
      - proxy-tier
#+end_src
...
#+begin_src yaml
  calibre-web-books:
    ...
    volumes:
      - ${VOLUMES_DIR}/calibre/books:/config
      - ${BOOKS_DIR}:/books
    environment:
      # application
      PUID: ${UID}
...
#+begin_src yaml
  calibre-web-manuals:
    ...
    volumes:
      - ${VOLUMES_DIR}/calibre/manuals:/config
      - ${MANUALS_DIR}:/books
    environment:
      # proxy
...
#+begin_src yaml
  calibre:
    ...
    volumes:
      - ${VOLUMES_DIR}/calibre/manager.books:/config
      - ${BOOKS_DIR}:/books
    environment:
      # app
      PUID: ${UID}
...
#+begin_src yaml
  webtrees:
    ...
    depends_on:
      - db_webtrees
    volumes:
      - ${VOLUMES_DIR}/webtrees/data:/var/www/webtrees/data
      - ${VOLUMES_DIR}/webtrees/themes:/var/www/webtrees/modules_v4
    environment:
      # proxy
...
#+begin_src yaml
  navidrome:
    ...
    environment:
      ...
      ND_LISTENBRAINZ_BASEURL: http://maloja:${MALOJA_LOCAL_PORT}/apis/listenbrainz/1
    networks:
      - proxy-tier
      - music
#+end_src
...
#+begin_src yaml
  maloja:
    <<: *service_default
    image: krateng/maloja:${MALOJA_VERSION}
    container_name: maloja
    hostname: maloja
    ports:
      - ${MALOJA_LOCAL_PORT}:42010
    volumes:
      - ${VOLUMES_DIR}/maloja/config:/config/config
      - ${VOLUMES_DIR}/maloja/data:/config/state
    environment:
      # app
      PUID: ${UID}
      PGID: ${GID}
      TZ: ${TIME_ZONE}
      MALOJA_LOCATION_TIMEZONE: ${TIME_ZONE}
    networks:
      - music
#+end_src
...
#+begin_src yaml
  snapcast:
    ...
    ports:
      ...
      - 1780:1780
    volumes:
      - ${VOLUMES_DIR}/snapcast/config:/config
      - ${VOLUMES_DIR}/snapcast/data:/data
      - ${VOLUMES_DIR}/snapcast/fifos:/audio
    environment:
      # app
      PUID: ${UID}
      PGID: ${GID}
...
#+begin_src yaml
  plex:
    ...
    ports:
      ...
      - 32414:32414/udp
    volumes:
      - ${VOLUMES_DIR}/plex/config:/config
      - ${VOLUMES_DIR}/plex/transcode:/transcode
...
#+begin_src yaml
  jellyfin:
    ...
    volumes:
      ...
      - ${VOLUMES_DIR}/jellyfin/cache:/cache
...
#+begin_src yaml
  asf:
    ...
    container_name: asf
    volumes:
      - ${VOLUMES_DIR}/archi-steam-farm:/app/config
    environment:
      # proxy
...
#+begin_src yaml
  find-my-device:
    <<: *service_default
    image: registry.gitlab.com/fmd-foss/fmd-server:${FMD_VERSION}
    container_name: find-my-device
    hostname: find-my-device
    ports:
      - ${FMD_LOCAL_PORT}:8080
    volumes:
      - ${VOLUMES_DIR}/find-my-device:/var/lib/fmd-server/db
    environment:
      # proxy
      VIRTUAL_HOST: ${FMD_HOST}
      VIRTUAL_PORT: 8080
      LETSENCRYPT_HOST: ${FMD_HOST}
      LETSENCRYPT_EMAIL: ${MY_EMAIL}
...
#+begin_src yaml
  open_webui:
    ...
    hostname: open_webui
    ports:
      - ${OPEN_WEBUI_LOCAL_PORT}:8080
    volumes:
      - ${AI_DIR}/open-webui:/app/backend/data
    environment:
      # proxy
      VIRTUAL_HOST: ${OPEN_WEBUI_HOST}
      VIRTUAL_PORT: 8080
      LETSENCRYPT_HOST: ${OPEN_WEBUI_HOST}
      LETSENCRYPT_EMAIL: ${MY_EMAIL}

      # app
      OLLAMA_BASE_URL: http://ollama:${OLLAMA_LOCAL_PORT}
...
#+begin_src yaml
  ollama:
    ...
    volumes:
      - ${AI_DIR}/ollama:/root/.ollama
      - ${AI_MODELS_DIR}/ollama:/root/.ollama/models
    networks:
      - hass
      - ai
#+end_src

** Speaches :speech:stt:tts:
#+begin_src yaml
  speaches:
    <<: *service_default
    build:
    ...
    healthcheck:
      ...
      start_period: 5s
    ports:
      - ${SPEACHES_LOCAL_PORT}:8000
    volumes:
      - ${AI_MODELS_DIR}/speaches:/home/ubuntu/.cache/huggingface/hub
    networks:
      - ai
#+end_src

** Immich Machine Learning :ai:photo:
#+begin_src yaml
  immich-machine-learning:
    ...
    volumes:
      - ${VOLUMES_DIR}/immich/cache:/cache
    networks:
      - immich
#+end_src
...
#+begin_src yaml
  jenkins:
    <<: *service_default
    image: jenkins/jenkins:${JENKINS_VERSION}-${JENKINS_JAVA_VERSION}
